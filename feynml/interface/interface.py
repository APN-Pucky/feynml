...
logger = logging.getLogger("MadFeynML_plugin.Interface")
...
def generate_directories_fks_async(i):
    ...
    for real_me in me.real_processes:
        MadFeynML_fks.find_os_divergences(real_me)
        real_me.os_matrix_elements = [
            helas_objects.HelasDecayChainProcess(
                os_amp
        ...
        os_couplings.extend(
            sum(
                [
                    c
                    for osme in real_me.os_matrix_elements
        ...
        os_lorentz.extend(
            sum([osme.get_used_lorentz() for osme in real_me.os_matrix_elements], [])
        )

    calls = curr_exporter.generate_directories_fks(
    ...
    return [
        calls,
        curr_exporter.fksdirs,
        max_loop_vertex_rank,
        ninitial,
        nexternal,
        processes,
        os_couplings,
...
class MadFeynMLInterface(master_interface.MasterCmd):
    ...
    def do_launch(self, line, *args, **opts):
        ...
        """
        raise MadFeynMLInterfaceError(
            "\nWith MadFeynML, the launch command must not be executed from the MG5_aMC shell.\n"
    ...
    def do_add(self, line, *args, **opts):
        ...
        """
        super(MadFeynMLInterface, self).do_add(line, *args, **opts)
        ...
        if not hasattr(self, "_fks_multi_proc") or not self._fks_multi_proc:
            logger.warning(
                "No NLO Process has been generated.\n To use MadFeynML, please generate a process with [QCD]"
            )
            return
        ...
        for born in self._fks_multi_proc["born_processes"]:
            for real in born.real_amps:
                self.n_os += MadFeynML_fks.find_os_divergences(real)
    ...
    def do_output(self, line):
        ...
        if not hasattr(self, "_fks_multi_proc") or not self._fks_multi_proc:
            ...
            super(MadFeynMLInterface, self).do_output(line)
            return
        elif self.n_os == 0:
            super(MadFeynMLInterface, self).do_output(line)
            return
        ...
        if self._export_format in ["NLO"]:
            ...
            self._curr_exporter = MadFeynML_exporter.MadFeynMLExporter(
                self._export_dir, to_pass
            )
        ...
        if (
            not force
            and not noclean
            and os.path.isdir(self._export_dir)
            ...
            if not self._curr_matrix_elements.get_matrix_elements():
                if group:
                    raise MadGraph5Error(
                        "Cannot group subprocesses when " + "exporting to NLO"
                    )
                else:
                    if not self.options["low_mem_multicore_nlo_generation"]:
                        # generate the code the old way
                        self._curr_matrix_elements = (
                            MadFeynML_fks.FKSHelasMultiProcessWithOS(
                                    ...
                                    sorted(
                                        list(
                                            set(
                                                ...
                                                for p in me.born_matrix_element.get(
                                                    "processes"
                                                )
                                            )
                                        )
                                        ...
                                        sorted(
                                            list(
                                                set(
                                                    ...
                                                    for p in fksreal.matrix_element.get(
                                                        "processes"
                                                    )
                                                )
                                            )
                        ...
                        # new NLO generation
                        self._curr_matrix_elements = fks_helas.FKSHelasMultiProcess(
                            self._fks_multi_proc,
                            loop_optimized=self.options["loop_optimized_output"],
                        )
        ...
        if self._export_format in ["NLO"]:
            ...
            for ime, me in enumerate(self._curr_matrix_elements.get("matrix_elements")):
                if not self.options["low_mem_multicore_nlo_generation"]:
                    ...
                    calls = calls + self._curr_exporter.generate_directories_fks(
                        me,
            ...
            if self.options["low_mem_multicore_nlo_generation"]:
                ...
                try:
                    ...
                    diroutputmap = pool.map_async(
                        generate_directories_fks_async, range(len(glob_directories_map))
                    ).get(9999999)
                ...
                for charac in ["nexternal", "ninitial"]:
                    proc_charac[charac] = self._curr_exporter.proc_characteristic[
                        charac
                    ]
                ...
                if len(ninitial_set) != 1:
                    raise MadGraph5Error(
                        "Invalid ninitial values: %s" % " ,".join(list(ninitial_set))
                    )
                ...
                max_loop_vertex_ranks = [
                    me.get_max_loop_vertex_rank()
                    for me in self._curr_matrix_elements.get_virt_matrix_elements()
                ]

            card_path = os.path.join(
                path, os.path.pardir, "SubProcesses", "procdef_mg5.dat"
            )

            if self.options["loop_optimized_output"] and len(max_loop_vertex_ranks) > 0:
            ...
            if self._generate_info:
                ...
                try:
                    ...
                    pass
            subproc_path = os.path.join(
                path, os.path.pardir, "SubProcesses", "initial_states_map.dat"
            )
